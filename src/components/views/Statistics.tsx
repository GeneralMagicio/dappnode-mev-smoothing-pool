...
import { fetchStatistics, fetchStatus } from '@/client/api/queryFunctions'
...
export function Statistics() {
  ...
  return (
    <div className="grid w-full grid-cols-1 items-center justify-center gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 lg:gap-6">
      ...
      />
      <AverageRewardsCard
        averageEthRewardWei={statisticsQuery.data?.avgBlockRewardWei}
        isError={statisticsQuery.isError}
        isLoading={statisticsQuery.isLoading}
        spAverageEthRewardWei={statisticsQuery.data?.avgBlockRewardWei}
      />
      <TotalRewardsCard
        isError={statisticsQuery.isError}
        isLoading={statisticsQuery.isLoading}
